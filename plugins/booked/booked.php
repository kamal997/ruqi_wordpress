...
/*
Plugin Name: Booked
Plugin URI: https://quickcal.co/booked-upgrade
Description: Powerful appointment booking made simple.
Version: 2.4.4
Author: QuickCal, BoxyStudio
Author URI: https://quickcal.co/booked-upgrade
Text Domain: booked
*/

define( 'BOOKED_VERSION', '2.4.4' );
...
$booked_update_check = PucFactory::buildUpdateChecker('https://quickcal.co/update/booked.json', __FILE__, 'booked');
	...
	class booked_plugin {
		...
		public function __construct() {
			...
			add_action('parent_file', array(&$this, 'booked_tax_menu_correction'));
                        ...
                        add_action('admin_notices', array(&$this, 'booked_outdated_notice' ));
                        add_action('wp_ajax_booked_dismiss_outdated_notice', array(&$this,'booked_dismiss_outdated_notice'));
                        add_action( 'admin_enqueue_scripts', array( &$this, 'booked_dismiss_outdated_notice_script' ) );
		...
		**********************/
		...
		public function add_menu() {
			...
			add_submenu_page('booked-appointments', esc_html__('Security updates','booked'), esc_html__('Security updates','booked'), 'manage_booked_options', 'booked-welcome', array(&$this, 'booked_welcome_content'));
		}
		...
		public function booked_no_profile_page_notice() {
			...
			if (current_user_can('manage_booked_options')):
                ...
                public function booked_dismiss_outdated_notice_script() {
                    if($this->booked_do_we_show_outdated_notice()){
                        wp_enqueue_script( 'jquery' );
                        wp_enqueue_script(
                            'booked-outdated',
                            BOOKED_PLUGIN_URL . '/assets/js/admin-notice.js',
                            array( 'jquery' ),
                            '1.0'
                        );
                    }
                }
                public function booked_dismiss_outdated_notice() {
                    
                    $cache_key = 'booked_outdated_notice_'. get_current_user_id();
                    $next_time_in_days = (INT)get_option($cache_key);
                    //we just showed the one in the setting
                    if(!$next_time_in_days){
                        update_option($cache_key, '14');//next after 14 days   
                    }
                    else if($next_time_in_days===14){
                        update_option($cache_key, '90');//next after 90 days   
                    }
                    else if($next_time_in_days>=90){
                        $show = true;
                        update_option($cache_key, $next_time_in_days+90);//next after every 90 days   

                    }
                    wp_die();
                }
                
                public function booked_do_we_show_outdated_notice(){
                    
                    $cache_key = 'booked_outdated_notice_'. get_current_user_id();
                    
                    $show = false;
                    $last_shown = (INT)get_option($cache_key);
                    
                    if(!$last_shown){
                        //we show the notice for the first time
                        $show = true;
                    }
                    elseif(!empty($last_shown)){
                        //we show it after two weeks, then 3 months from the time the plugin was last updated
                        $diff = (time() - filemtime(BOOKED_PLUGIN_DIR))/60/60/24;
                        if($last_shown===14 && $diff>=14){
                            $show = true;
                        }
                        else if($last_shown===90 && $diff>=90){
                            $show = true;
                        }
                    }
                ...
                public function booked_outdated_notice() {
                    
			if ( current_user_can('activate_plugins') && current_user_can('install_plugins')):
                            
                            if($this->booked_do_we_show_outdated_notice()){
                                $action_url = 'https://quickcal.co/booked_upgrade';
                                $button_label = __('More Info', 'booked');
                                $button = '<a href="' . $action_url . '" class="button-secondary" target="_blank">' . $button_label . '</a>';

                                $action_url_2 = 'https://quickcal.co/booked-upgrade';
                                $button_label_2 = __('More Info', 'booked');
                                $button_2 = '<a href="' . $action_url_2 . '" class="button-primary" target="_blank">' . $button_label_2 . '</a>';

                                $class = 'booked-outdated-notice notice notice-warning is-dismissible';
                                $message = sprintf(__('<p><strong>The Booked plugin is no longer available.</strong><br> To continue to receive security updates, new features, and premium support, we recommend upgrading to QuickCal for a seamless transition.</p> %s'), $button_2);
                                $img = '<img src="'.BOOKED_PLUGIN_URL.'/templates/images/quickcal-badge.png'.'" height="40">';
                                echo '<style>'
                                        . '.booked-outdated-notice{'
                                        . 'border-color: #A40E4C;'
                                        . '}'
                                        . '.booked-outdated-notice > div{'
                                        . 'display: flex; align-items: center; gap: 10px;'
                                        . '}'
                                        . '.wp-core-ui .booked-outdated-notice .button-primary {'
                                        . 'background: #2C2C54;border-color: #2C2C54;'
                                        . '}'
                                . '</style>';
                                printf('<div class="%1$s" data-dismiss-action="booked_dismiss_outdated_notice"><div>'.$img.'%2$s</div></div>', esc_attr($class), wp_kses_post($message));
                            }
		...
		**********************/
		...
		public function admin_styles() {
			...
			if (in_array($current_page,$this->booked_screens) || $screen->id == 'dashboard'):
				...
				wp_enqueue_style('booked-admin', BOOKED_PLUGIN_URL . '/dist/booked-admin.css', array(), filemtime(BOOKED_PLUGIN_DIR. '/dist/booked-admin.css'));
			endif;
...
if(class_exists('booked_plugin')) {
	...
	if(isset($booked_plugin)) {
		...
		function booked_custom_links($links) {
			...
			$custom_links[] = '<a href="'.trailingslashit(get_admin_url()).'admin.php?page=booked-welcome">'.esc_html__('Security updates','booked').'</a>';
