<div class="booked-settings-prewrap">
	...
	if (!current_user_can('manage_booked_options') && $booked_none_assigned):
		...
		echo '<div style="text-align:center;">';
			echo '<br><br><h3>'.esc_html__('لا توجد أي تقاويم مخصصة لك.','booked').'</h3>';
			echo '<p>'.esc_html__('يرجى التواصل مع إدارة الموقع للحصول على تقويم مخصص لك.','booked').'</p>';
		echo '</div>';

	else:

		?><div class="topSavingState savingState"><i class="fa-solid fa-circle-notch fa-spin"></i>&nbsp;&nbsp;<?php esc_html_e('يتم التحديث، يرجى الانتظار...','booked'); ?></div>
		<div class="booked-settings-title"><?php esc_html_e('المواعيد المعلقة','booked'); ?></div>

		<div class="booked-pending-cap bookedClearFix">
			<button class="delete-past button"><?php esc_html_e('حذف المواعيد المنقضية','booked'); ?></button>
			<button style="float:right; margin-left:10px;" class="approve-all button button-primary"><?php esc_html_e('قبول الكل','booked'); ?></button>
			<button style="float:right;" class="delete-all button"><?php esc_html_e('حذف الكل','booked'); ?></button>
		</div><?php
		...
		echo '<div class="booked-pending-appt-list">';
			...
			*/
					...
					echo '<div class="pending-appt bookedClearFix'.($late_date > $date_to_compare ? ' passed' : '').'" data-appt-id="'.$appt['post_id'].'">';
						...
						if (!isset($appt['guest_name'])):
							...
							if (isset($user_info->ID)):
								...
								$user_display = esc_html__('(هذا المستخدم لم يعد موجودًا)','booked');
							endif;
						else :
						...
						echo '<span class="appt-block" data-appt-id="'.$appt['post_id'].'">';
							...
							echo '<button data-appt-id="'.$appt['post_id'].'" class="approve button button-primary">'.esc_html__('قبول','booked').'</button>';
							echo '<button class="delete button">'.esc_html__('حذف','booked').'</button>';

							echo $user_display;

							echo '<br>';
							if ($late_date > $date_to_compare): echo '<span class="late-appt">' . esc_html__('هذا الموعد قد انتهى.','booked') . '</span><br>'; endif;
							if ($calendar_owner_name): echo '<i class="fa-solid fa-user"></i>&nbsp;&nbsp;'.sprintf(esc_html__('مخصص لـ %s','booked'),$calendar_owner_name).'<br>'; endif;
			...
			echo '<div class="pending-appt'.(!empty($appointments_array) ? ' no-pending-message' : '').'">';
				echo '<p style="text-align:center; margin:3em 0 2em">'.esc_html__('لا توجد مواعيد معلقة.','booked').'</p>';
			echo '</div>';
