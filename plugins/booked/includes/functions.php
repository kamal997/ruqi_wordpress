	...
	echo '<div class="booked-appt-list">';
		...
		*/
		...
		if ($todays_defaults){
				...
				*/
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($timeslot_parts[0] == '0000' && $timeslot_parts[1] == '2400') :
                        $timeslotText = esc_html__('طوال اليوم', 'booked');
                    else :
                        $time_format = get_option('time_format'); // صيغة الوقت الافتراضية
                        ...
                        $timeslotText = $start_time . ' ' . $start_period . (!get_option('booked_hide_end_times') ? ' &ndash; ' . $end_time . ' ' . $end_period : '');
                    endif;
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$button_text = (!$spots_available || !$available ? esc_html__('غير متاح', 'booked') : esc_html(_x('حجز موعد', 'Book a Single Appointment', 'booked')));
						$html = '<div class="timeslot bookedClearFix'.($title && $only_titles == true ? ' booked-hide-time' : '').($hide_available_count || !$available ? ' timeslot-count-hidden' : '').(!$available ? ' timeslot-unavailable' : '').($title ? ' has-title ' : '').'">';
							...
							$html .= '<span class="timeslot-time'.($public_appointments ? ' booked-public-appointments' : '').'">';
								...
								if (!$hide_available_count):
									$html .= '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">';
										$html .= sprintf( _n( '%d موعد متاح', '%d مواعيد متاحة', $spots_available, 'booked' ), $spots_available );
									$html .= '</span>';
								endif;
							...
							$html .= '</span>';
							$html .= '<span class="timeslot-people"><button data-calendar-id="'.$calendar_id.'" data-title="'.esc_attr($title).'" data-timeslot="'.$timeslot.'" data-date="'.$date.'" class="new-appt button"'.(!$spots_available || !$available ? ' disabled' : '').'>'.apply_filters('booked_button_book_appointment', '<span class="button-text">' . $button_text . '</span>').'&nbsp;'.( $title ? '<span class="timeslot-mobile-title">'.esc_html($title).'</span>' : '' ).'<span class="button-timeslot">'.apply_filters('booked_fe_mobile_timeslot_button',$timeslotText,$this_timeslot_timestamp,$timeslot,$calendar_id).'</span>'.( !$hide_available_count ? '<span class="spots-available' . ( $spots_available == 0 ? ' empty' : '' ) . '">' . sprintf( esc_html( _n( '%d space available', '%d spaces available', $spots_available, 'booked' ) ), $spots_available ) . '</span>' : '' ).'</button></span>';
			...
			if (!$temp_count):
                echo '<p>'.esc_html__('تم طلب موعدك بنجاح! سيتم إشعارك عبر البريد الإلكتروني بمجرد تأكيد الموعد. يمكنك متابعة حالة مواعيدك من خلال لوحة التحكم الخاصة بك.','booked').'</p>';
		...
		*/
		...
		echo '<h2><span>' . sprintf( _n( esc_html(_x( 'موعد متاح بتاريخ %s', 'Single Appointment', 'booked' )), esc_html(_x( 'مواعيد متاحة بتاريخ %s', 'Multiple Appointments', 'booked' )), $total_available ), '</span><strong>'.$date_display.'</strong><span>') . '</span></h2>';
		echo $appt_list_html;
	...
	echo '<div class="booked-appt-list" data-list-date="'.$date.'" data-min-date="'.($prevent_before ? date_i18n('Y-m-d',strtotime($prevent_before)) : $earliest_date).'" data-max-date="'.($prevent_after ? date_i18n('Y-m-d',strtotime($prevent_after)) : false).'">';
		...
		*/
		...
		echo '<div class="booked-list-view-nav" data-calendar-id="'.$calendar_id.'">';
			if ($showing_prev):
				echo '<button data-date="'.date_i18n('Y-m-d',strtotime($prev_day)).'" class="booked-list-view-date-prev bb-small"><i class="fa-solid fa-arrow-left"></i>&nbsp;&nbsp;'.esc_html__('السابق    ','booked').'</button>';
			endif;
			if ($showing_next):
				echo '<button data-date="'.date_i18n('Y-m-d',strtotime($next_day)).'" class="booked-list-view-date-next bb-small">'.esc_html__('التالي','booked').'&nbsp;&nbsp;<i class="fa-solid fa-arrow-right"></i></button>';
			endif;
...
$html .= '<span class="timeslot-people"><button data-calendar-id="'.$calendar_id.'" data-title="'.esc_attr($title).'" data-timeslot="'.$timeslot.'" data-date="'.$date.'" class="new-appt button"'.(!$spots_available || !$available ? ' disabled' : '').'>'.apply_filters('booked_button_book_appointment', '<span class="button-text">' . $button_text . '</span>').'&nbsp;'.( $title ? '<span class="timeslot-mobile-title">'.esc_html($title).'</span>' : '' ).'<span class="button-timeslot">'.apply_filters('booked_fe_mobile_timeslot_button',$timeslotText,$this_timeslot_timestamp,$timeslot,$calendar_id).'</span>'.( !$hide_available_count ? '<span class="spots-available' . ( $spots_available == 0 ? ' empty' : '' ) . '">' . sprintf( esc_html( _n( '%d space available', '%d spaces available', $spots_available, 'booked' ) ), $spots_available ) . '</span>' : '' ).'</button></span>';
			...
			if (!$temp_count):
				...
				echo '<p>'.esc_html__('لا توجد مواعيد متاحة لهذا اليوم.','booked').'</p>';
		...
		*/
		...
		echo '<div class="booked-appt-list-header">';
            echo '<h2'.(!$showing_prev ? ' class="booked-no-prev"' : '').'><span>'.sprintf(_n('موعد متاح بتاريخ %s', 'مواعيد متاحة بتاريخ %s', $total_available, 'booked'), '</span><strong>'.$date_display.'</strong>').'</h2>';
...
function booked_reset_password($user_login){
    ...
    $message = esc_html__('طلب شخص ما إعادة تعيين كلمة المرور للحساب التالي:','booked') . "\r\n\r\n";
    $message .= network_home_url( '/' ) . "\r\n\r\n";
    $message .= sprintf(esc_html__('اسم المستخدم: %s','booked'), $user_login) . "\r\n\r\n";
    $message .= esc_html__('إذا كان هذا خطأً، يمكنك تجاهل هذه الرسالة ولن يحدث شيء.','booked') . "\r\n\r\n";
    $message .= esc_html__('لإعادة تعيين كلمة المرور، قم بزيارة الرابط التالي:','booked') . "\r\n\r\n";
    ...
    $title = sprintf( esc_html__('[%s] إعادة تعيين كلمة المرور','booked'), $blogname );
		...
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'update-user' ) {
		    ...
		    if (isset($_POST['pass1']) && isset($_POST['pass2']) && $_POST['pass1'] && $_POST['pass2'] ) {
		        if ( $_POST['pass1'] == $_POST['pass2'] )
		            ...
		            $error[] = esc_html__('كلمات المرور التي أدخلتها غير متطابقة. لم يتم تحديث كلمة المرور الخاصة بك.', 'profile');
		    }
		    ...
		    if ( isset( $_POST['email'] ) ){
		        ...
		        if (!is_email(esc_attr( $_POST['email'] )))
		            $error[] = esc_html__('البريد الإلكتروني الذي أدخلته غير صالح. يرجى المحاولة مرة أخرى.', 'profile');
		        elseif( $email_exists && $email_exists != $booked_current_user->ID )
		            $error[] = esc_html__('هذا البريد الإلكتروني مستخدم بالفعل من قبل مستخدم آخر. جرب بريدًا إلكترونيًا مختلفًا.', 'profile');
		        else{
	        ...
	        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] == 0):
			    ...
			    if (isset($avatar,$_POST['avatar_nonce']) && $avatar && wp_verify_nonce( $_POST['avatar_nonce'], 'avatar_upload' )) {
				    ...
				    if ( is_wp_error( $attachment_id ) ) {
				        $error[] = esc_html__('حدث خطأ أثناء تحميل الصورة الرمزية.','booked');
				    } else {
			    	...
			    	$error[] = esc_html__('فشل فحص أمان رفع الصورة الرمزية.','booked');
...
function booked_profile_content_appointments(){
	...
	if ( apply_filters( 'booked_sessions_enabled', true ) ){
		if (isset($_SESSION['appt_requested']) && isset($_SESSION['new_account'])){
			...
			echo '<p class="booked-form-notice">'.esc_html__('تم طلب موعدك! لقد قمنا أيضًا بإنشاء حساب لك. تم إرسال معلومات تسجيل الدخول الخاصة بك عبر البريد الإلكتروني. عند تسجيل الدخول، يمكنك عرض مواعيدك القادمة أدناه. تأكد من تغيير كلمة المرور الخاصة بك إلى شيء يسهل تذكره باستخدام علامة التبويب "تعديل الملف الشخصي" أعلاه.','booked').'</p>';
			...
			if ($appointment_default_status == 'draft'):
				echo '<p class="booked-form-notice">'.esc_html__('تم طلب موعدك! سيتم تحديثه أدناه إذا تمت الموافقة عليه.','booked').'</p>';
			else :
				echo '<p class="booked-form-notice">'.esc_html__('تمت إضافة موعدك إلى التقويم الخاص بنا!','booked').'</p>';
			endif;
...
function booked_profile_content_edit(){
	...
	echo '<h4><i class="fa-solid fa-pen-to-square" style="position:relative; top:-1px;"></i>&nbsp;&nbsp;'.esc_html__('تعديل الملف الشخصي','booked').'</h4>'; ?>
