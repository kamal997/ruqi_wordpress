...
function booked_add_to_calendar_button($dates,$cf_meta_value){
	if (!get_option('booked_hide_google_link',false)):
		...
		?><div title="<?php esc_attr_e('Add to Calendar','booked'); ?>" class="addeventatc atc-style-booked google-cal-button">
		    ...
		    <span class="title"><?php echo sprintf(esc_html__('موعد مع %s','booked'),get_bloginfo('name')); ?></span>
...
function booked_registration_validation( $email, $password, $captcha_value = false, $captcha_from_user = false )  {
	...
	if ($captcha_value):
		if ($captcha_value != $captcha_from_user):
			$reg_errors->add('captcha', esc_html__('النص الذي أدخلته لا يطابق الصورة.','booked'));
		else :
	...
	if ( !$email || !$password ) {
	    $reg_errors->add('field', esc_html__('جميع الحقول مطلوبة للتسجيل.','booked'));
	}

	if ( username_exists( $email ) ) {
    	$reg_errors->add('user_name', esc_html__('اسم المستخدم هذا موجود بالفعل.','booked'));
    }

    if ( ! validate_username( $email ) ) {
	    $reg_errors->add( 'username_invalid', esc_html__('هذا الاسم غير صالح.','booked'));
	}

    if ( !is_email( $email ) ) {
	    $reg_errors->add( 'email_invalid', esc_html__('عنوان البريد الإلكتروني هذا غير صالح.','booked'));
	}

	if ( email_exists( $email ) ) {
	    $reg_errors->add( 'email', esc_html__('هذا البريد الإلكتروني مستخدم بالفعل.','booked'));
	}
...
function booked_complete_registration() {
    ...
    if ( 1 > count( $reg_errors->get_error_messages() ) ) {
        ...
        return '<p class="booked-form-notice"><strong>'.esc_html__('نجاح!','booked').'</strong><br />'.esc_html__('تم إكمال التسجيل، يرجى التحقق من بريدك الإلكتروني للحصول على معلومات تسجيل الدخول.','booked').'</p>';
...
function booked_registration_form($name, $surname, $email, $password){
	...
	?><form action="" method="post" class="wp-user-form">
		...
		<p class="name">
			<label for="name"><?php echo ( $name_requirements == 'require_surname' ? esc_html__('الاسم الأول','booked') : esc_html__('اسمك','booked') ); ?></label>
