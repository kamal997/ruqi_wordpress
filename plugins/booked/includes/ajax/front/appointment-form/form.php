	...
	<form name="customerChoices" action="" id="customerChoices" class="bookedClearFix"<?php echo ( !get_option('users_can_register') ? ' style="display:none;"' : '' ); ?>>
		...
		<?php if ( get_option('users_can_register') ): ?>
			<div class="field">
				<span class="checkbox-radio-block">
					...
					<label for="customer_new"><?php esc_html_e('طالب جديد','booked'); ?></label>
				</span>
			</div>
		<?php endif; ?>
		...
		<div class="field">
			<span class="checkbox-radio-block">
				...
				<label for="customer_current"><?php esc_html_e('طالب حالي','booked'); ?></label>
			</span>
		</div>
	</form>
	...
	<div class="condition-block customer_choice<?php echo ( !get_option('users_can_register') && !is_user_logged_in() ? ' default' : '' ); ?>" id="condition-current">
		...
		<form id="ajaxlogin" action="" method="post" data-date="<?php echo $first_booking['date']; ?>" data-title="<?php echo $first_booking['title']; ?>" data-timeslot="<?php echo $first_booking['timeslot']; ?>" data-calendar-id="<?php echo $first_booking['calendar_id']; ?>">
			<div class="cf-block">
				...
				<?php if ( !is_multisite() ): ?>
				<a href="#" class="booked-forgot-password"><?php esc_html_e( 'نسيت كلمة المرور.', 'booked' ); ?></a>
				<?php endif; ?>

			</div>

			<div class="field">
				<input name="submit" type="submit" class="button button-primary" value="<?php esc_html_e('تسجيل الدخول', 'booked') ?>">
				<button class="cancel button"><?php esc_html_e('إلغاء','booked'); ?></button>
			</div>
		</form>
		...
		<?php if ( !is_multisite() ): ?>
			<form id="ajaxforgot" action="" method="post">
				...
				<div class="field">
					<input name="submit_forgot" type="submit" class="button button-primary" value="<?php esc_html_e('إعادة تعيين كلمة المرور', 'booked') ?>">
					<button class="booked-forgot-goback button"><?php esc_html_e('الرجوع','booked'); ?></button>
				</div>
			</form>
		<?php endif; ?>
...
<div class="condition-block customer_choice<?php echo ( $guest_booking || get_option('users_can_register') && !is_user_logged_in() || is_user_logged_in() ? ' default' : '' ); ?>" id="condition-new">
	<form action="" method="post" id="newAppointmentForm">
		...
		// User limit reached
		if ( $reached_limit ) {
			$error_message = sprintf(_n("عذرًا، لقد وصلت إلى حد المواعيد. يمكن لكل مستخدم حجز %d موعد فقط في وقت واحد.","عذرًا، لقد وصلت إلى حد المواعيد. يمكن لكل مستخدم حجز %d مواعيد فقط في وقت واحد.", $appointment_limit, "booked" ), $appointment_limit);
		}
		...
		if ( $will_reached_limit && ! $reached_limit ) {
			$error_message = sprintf(esc_html__("عذرًا، ستقوم بحجز مواعيد أكثر مما يسمح به النظام. يمكن لكل مستخدم حجز %d مواعيد فقط في وقت واحد.", "booked" ), $appointment_limit);
		}
		...
		if ( $is_user_logged_in && ! $error_message ) {
			$msg = sprintf( _n( 'أنت على وشك طلب موعد لـ %s.', 'أنت على وشك طلب مواعيد لـ %s.', $total_appts, 'booked' ), '<em>' . booked_get_name( $booked_current_user->ID ) . '</em>' ) . ' ' . _n( 'يرجى المراجعة والتأكيد أنك ترغب في طلب الموعد التالي:', 'يرجى المراجعة والتأكيد أنك ترغب في طلب المواعيد التالية:', $total_appts, 'booked' );
			echo wpautop( $msg );
		}

		// If there aren't any errors, and the user isn't logged in
		if ( ! $is_user_logged_in && ! $error_message ) {
			$msg = _n( 'يرجى التأكيد أنك ترغب في طلب الموعد التالي:', 'يرجى التأكيد أنك ترغب في طلب المواعيد التالية:', $total_appts, 'booked' );
			echo wpautop( $msg );
		}
		...
		if ( ! $is_user_logged_in && ! $error_message && class_exists('ReallySimpleCaptcha') ) : ?>
			...
			?>
			<p class="captcha">
				<label for="captcha_code"><?php esc_html_e('يرجى إدخال النص التالي:','booked'); ?></label>
		...
		<div class="field">
			<?php if ( $error_message ): ?>
				<button class="cancel button"><?php esc_html_e('موافق','booked'); ?></button>
			<?php else: ?>
				<input type="submit" id="submit-request-appointment" class="button button-primary" value="<?php echo ( $new_appointment_default == 'draft' ? esc_html( _n( 'طلب موعد', 'طلب مواعيد', $total_appts, 'booked' ) ) : esc_html( _n( 'حجز موعد', 'حجز مواعيد', $total_appts, 'booked' ) ) ); ?>">
				<button class="cancel button"><?php esc_html_e('إلغاء','booked'); ?></button>
			<?php endif; ?>
		</div>
	</form>
