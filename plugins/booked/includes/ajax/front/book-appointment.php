...
if ($appt_is_available):
	...
	if ($customer_type == 'guest'):
		...
		if ( $name_requirements == 'require_surname' && !$surname ):
			...
			echo 'error###'.esc_html__('اسمك الكامل مطلوب لحجز موعد..','booked');
			...
			if ($email && is_email($email) && $name || !$email && !$email_required && $name):
				...
				if ($email && !is_email($email)):
					$errors[] = esc_html__('عنوان البريد الإلكتروني الذي أدخلته غير صالح.','booked');
				elseif ($email_required && !$email):
					$errors[] = esc_html__('سمك وعنوان بريد إلكتروني صالح مطلوبان لحجز موعد.','booked');
				elseif (!$name):
					$errors[] = esc_html__('اسمك مطلوب لحجز موعد.','booked');
				else:
					$errors[] = esc_html__('حدث خطأ غير معروف.','booked');
				endif;
...
elseif ( $is_new_registration ):
	...
	if ( $name_requirements == 'require_surname' && !$surname ):
		...
		if (empty($errors)):
	        ...
	        echo 'success###' . esc_html__('تم التسجيل بنجاح..','booked');
...
',$errors);
	...
	$error_message = apply_filters(
		...
		esc_html__('عذرًا، قام شخص آخر بحجز هذا الموعد قبلك. يُرجى اختيار وقت آخر للحجز.','booked')
	);
