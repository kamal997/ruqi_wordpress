...
class BookedShortcodes {
	...
	public function booked_appointments_shortcode($atts = null, $content = null) {
		...
		if ( is_user_logged_in() ):
				...
				echo '<div class="booked-profile-appt-list">';
					...
					if ($historic):
						if ($total_appts):
							...
							echo '<p class="booked-no-margin">'.esc_html__('لا توجد مواعيد سابقة.','booked').'</p>';
						endif;
					else:
						if ($total_appts):
							echo '<h4><span class="count">' . number_format($total_appts) . '</span> ' . _n('موعد قادم',' مواعيد قادمة',$total_appts,'booked') . '</h4>';
						else:
							echo '<p class="booked-no-margin">'.esc_html__('لا توجد مواعيد قادمة.','booked').'</p>';
						endif;
					endif;
					...
					foreach($appointments_array as $appt):
						...
						if ($date_display == $today){
							$date_display = esc_html__('اليوم','booked');
							$day_name = '';
						} else {
						...
						$timeslotText = '';
						$status = ($appt['status'] != 'publish' && $appt['status'] != 'future' ? esc_html__('قيد الانتظار','booked') : esc_html__('موافق عليه','booked'));
						...
						if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
							...
							if ($only_titles && !$ts_title || !$only_titles):
								$timeslotText = (!get_option('booked_hide_end_times') ? esc_html__('من','booked').' ' : esc_html__('في','booked').' ') . $time_start . (!get_option('booked_hide_end_times') ? ' &ndash; '.$time_end : '');
							endif;
						...
						echo '<span class="appt-block bookedClearFix '.(!$historic ? $status_class : 'approved').'" data-appt-id="'.$appt['post_id'].'">';
                            ...
                            echo '<div style="display: flex;justify-content: space-between;align-items: center;"><div>';

							echo (!empty($appt['calendar_id']) ? '<span><i class="fa-solid fa-calendar-days pd-l"></i><strong>'.esc_html__('التقويم','booked').':</strong> '.$appt['calendar_id'][0]->name.'</span><br>' : '');

							echo '<span><i class="fa-solid fa-clock pd-l"></i>'.($ts_title ? '<strong>'.$ts_title.':</strong>&nbsp;&nbsp;' : '').$day_name.$date_display.'&nbsp;&nbsp;' . $timeslotText . '</span></div>';
							...
							echo ($cf_meta_value ? '<br><i class="fa-solid fa-circle-info"></i><a href="#" class="booked-show-cf">'.esc_html__('معلومات إضافية.','booked').'</a><div class="cf-meta-values-hidden">'.$cf_meta_value.'</div>' : '');
							...
							if (!$historic):
								...
								if ( apply_filters('booked_shortcode_appointments_allow_cancel', true, $appt['post_id']) && !get_option('booked_dont_allow_user_cancellations',false) ) { if ( $appt_date_time >= $date_to_compare ) { echo '<a href="#" data-appt-id="'.$appt['post_id'].'" class="cancel">'.esc_html__('إلغاء الموعد.','booked').'</a>'; } }
			...
			return '<p>'.esc_html__('يُرجى تسجيل الدخول لعرض مواعيدك القادمة.','booked').'</p>';
