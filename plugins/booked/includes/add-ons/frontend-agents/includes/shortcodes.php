		...
		if (is_user_logged_in() && current_user_can('edit_booked_appointments')):
			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="booked-profile-page" class="booked-shortcode">'; endif;
				...
				echo '<div class="booked-profile-appt-list booked-fea-appt-list">';
					...
					if ($pending):
                        if ($total_appts):
                            echo '<h4>'.sprintf(_n('%s موعد قيد الانتظار','%s مواعيد قيد الانتظار',$total_appts,'booked'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
                        else:
                            echo '<p class="booked-no-margin">'.__('لا توجد مواعيد قيد الانتظار.','booked').'</p>';
                        endif;
                    elseif ($historic):
                        if ($total_appts):
                            echo '<h4>'.sprintf(_n('%s موعد سابق','%s مواعيد سابقة',$total_appts,'booked'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
                        else:
                            echo '<p class="booked-no-margin">'.__('لا توجد مواعيد سابقة.','booked').'</p>';
                        endif;	
                    else:
                        if ($total_appts):
                            echo '<h4>'.sprintf(_n('%s موعد قادم','%s مواعيد قادمة',$total_appts,'booked'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
                        else:
                            echo '<p class="booked-no-margin">'.__('لا توجد مواعيد قادمة.','booked').'</p>';
                        endif;
                    endif;
