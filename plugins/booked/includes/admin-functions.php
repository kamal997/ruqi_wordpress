...
function booked_parse_readme_changelog( $readme_url = false, $title = false ){
	...
	$whats_new_title = '<h3>' . ( $title ? esc_html( $title ) : apply_filters( 'booked_whats_new_title', esc_html__( "Changelog", "booked" ) ) ) . '</h3>';
...
function booked_render_interval_select($select_name,$placeholder){
	...
	<option value="60" selected><?php esc_html_e('كل ساعة','booked'); ?></option>
	<option value="90"><?php esc_html_e('كل ساعة و30 دقيقة','booked'); ?></option>
	<option value="120"><?php esc_html_e('كل ساعتين','booked'); ?></option>
	<option value="45"><?php esc_html_e('كل 45 دقيقة','booked'); ?></option>
	<option value="30"><?php esc_html_e('كل 30 دقيقة','booked'); ?></option>
	<option value="20"><?php esc_html_e('كل 20 دقيقة','booked'); ?></option>
	<option value="15"><?php esc_html_e('كل 15 دقيقة','booked'); ?></option>
	<option value="10"><?php esc_html_e('كل 10 دقائق','booked'); ?></option>
	<option value="5"><?php esc_html_e('كل 5 دقائق','booked'); ?></option>
	<?php echo '</select>';
	...
	echo '<div class="booked-appt-list">';
		...
		*/
		...
		if ($tabbed):
			...
			$tab_title = esc_html__('المواعيد لـ','booked');
			$is_first_tab = true;
			if ( !get_option('booked_hide_default_calendar') && current_user_can('manage_booked_options') ):
			...
			foreach($calendars as $calendar):
				?><div id="bookedCalendarAppointmentsTab-<?php echo $calendar->term_id; ?>" class="bookedAppointmentTab<?php if ( $calendar_id == $calendar->term_id || !$calendar_id && $is_first_tab): ?> active<?php endif; ?>"><?php
					...
					$tab_title = sprintf(esc_html__('%s المواعيد لـ','booked'),'<strong>'.$calendar->name.'</strong>');
...
function booked_admin_calendar_date_loop($date,$time_format,$date_format,$calendar_id = false,$tab_title='',$tabbed = false,$calendars = false){
	...
	*/
		...
		echo '<p class="booked-none-available">'.esc_html__('لا توجد مواعيد متاحة لهذا اليوم.','booked').' <a href="'.get_admin_url(null,'admin.php?page=booked-settings#defaults').'">'.esc_html__('هل ترغب في إضافة مواعيد؟','booked').'</a></p>';
	}
